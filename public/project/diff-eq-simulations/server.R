function(input,output){
  ...
  observeEvent(input$enter,{
    ...
    time_steps <- switch(as.numeric(input$acc),1000,3000,10000,30000)
  ...
  anim_save(filename = "vec_field.gif",animation = a,path = path_to_main)
  output$gif2 <- renderImage(list(src =paste0(path_to_main,"/vec_field.gif"),contentType = 'image/gif' ),deleteFile = T)
